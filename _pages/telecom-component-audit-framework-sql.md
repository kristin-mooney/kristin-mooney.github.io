...
--
-- PURPOSE
--   End-to-end component order audit pipeline. Reconciles
--   component records across two downstream audit paths,
--   validates billing accuracy by charge type, matches failed
--   components to downstream rejects, applies a waterfall
--   risk-tier assessment, and materializes both a detail table
--   and an aggregated Power BI summary table.
--
-- STEP OVERVIEW
...
--   Step 5  Materialize detail table + risk tiers + summary table
--   Step 6  Truncate temp tables
--
-- DATA SOURCES
--   aud.system1_to_system2                          System 1 vs System 2 audit
--   aud.system1_to_system3                          System 1 vs System 3 audit
--   rpt.invoice_detail                              Billing actuals
--   ref.component_issue_risk_rootcause_solutions    Manual lookup/reference table
--
-- OUTPUT TABLES
--   aud.component_audit_results    Detail -- one row per component
--   aud.component_audit_summary    Aggregated Power BI feed
--
-- EXECUTION
--   EXEC [aud].[sp_component_audit_details_and_summary_tables]
--
-- BUGS FIXED IN PORTFOLIO VERSION (v1.1)
--   Bug  1  Step 3 USAGE correctly billed WHERE clause:
--            a.component = 'ACTIVE' -> a.component_status = 'ACTIVE'
--            (was comparing component ID field to a status value)
--   Bug  2  Step 3 NRC correctly billed WHERE clause:
--            a.component-status -> a.component_status (hyphen -> underscore)
--   Bug  3  Step 3 NRC correctly billed JOIN:
--            b.acct_no -> b.account (masked proprietary field reference)
--   Bug  4  Step 3 MRC disconnect incorrectly billed JOIN:
--            b.acct_no -> b.account (masked proprietary field reference)
--   Bug  5  Step 3 NRC incorrectly billed SET:
--            b.Aaccount -> b.account (casing/typo)
--   Bug  6  Step 4 System 3 reject INSERT:
--            @ssis_run_id -> @run_id (undeclared variable)
--   Bug  7  Step 4 System 2 reject final INSERT:
--            leading stray comma on SUM(MOU) line removed (syntax error)
--   Bug  8  Step 4 System 3 JOIN:
--            tmp.components_audit_results -> tmp.component_audit_results
--            (extra 's' in table name)
--   Bug  9  Step 4 System 2 final INSERT:
--            @ssis_run_id -> @run_id (undeclared variable)
--   Bug 10  Step 5 detail INSERT column list:
--            compnent -> component (typo)
--   Bug 11  Step 5 detail INSERT column alias:
--            assesement -> assessment (typo)
--   Bug 12  Step 5 summary INSERT:
--            @ssis_run_id -> @run_id (undeclared variable)
--
-- NOTE
